...
int scoped_main(int argc, char *argv[])
{
	...
	try
	{
		...
		else if (command == "descifra")
		{
			// Descifra texto cifrado con Triple-DES (DES-EDE) en modo CBC
			// Modelo basado en la opción "t" ya existente
			char passPhrase[MAX_PHRASE_LENGTH];
			...
			std::cout << "\nTexto cifrado (hex): ";
			std::cin.getline(ciphertext, sizeof(ciphertext));

			std::string decrypted = DecryptString(ciphertext, passPhrase);
			std::cout << "\nTexto descifrado: " << decrypted << std::endl;
			}

		else if (command == "cifra_fichero")
		{
			// Cifra fichero con Triple-DES (DES-EDE) en modo CBC
			// Uso: cryptest.exe cifra_fichero <entrada> <salida>
			if (argc < 4)
			{
				std::cerr << "Uso: cryptest cifra_fichero <fichero_entrada> <fichero_salida>\n";
				return 1;
			}
			...
			EncryptFile(argv[2], argv[3], passPhrase);
			std::cout << "Fichero cifrado guardado en: " << argv[3] << std::endl;
			}

		else if (command == "descifra_fichero")
		{
			// Descifra fichero con Triple-DES (DES-EDE) en modo CBC
			// Uso: cryptest.exe descifra_fichero <entrada> <salida>
			if (argc < 4)
			{
				std::cerr << "Uso: cryptest descifra_fichero <fichero_cifrado> <fichero_salida>\n";
				return 1;
			}
			...
			DecryptFile(argv[2], argv[3], passPhrase);
			std::cout << "Fichero descifrado guardado en: " << argv[3] << std::endl;
			}
		else if (command == "h" || command == "X-help")
		{
			FileSource usage(DataDir("TestData/usage2.dat").c_str(), true, new FileSink(std::cout));
